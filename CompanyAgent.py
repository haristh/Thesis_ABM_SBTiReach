...
class CompanyAgent(mesa.Agent):
    ...
    def rewire(self):
        ...
        # Choose a potential new neighbor to connect to
        ...
        #potential_new_neighbors = sorted(set(self.model.schedule.agents), key=lambda x: x.unique_id) - set(current_neighbors) - {self} #REMOVE SORTED AFTER DEBUGGING

        
        if not potential_new_neighbors:
            return
    
        new_neighbor = self.model.random.choice(list(potential_new_neighbors))
    ...
    def step(self):
        ...
        # Updating work towards submission/ setting target
            ...
            if self.commitment_duration > self.model.max_comm_duration:
                ...
                if self.scheduling > self.model.random.random():
